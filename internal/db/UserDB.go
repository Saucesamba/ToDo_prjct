...
func CreateUser(db *sql.DB, user *models.User) (int, error) {
	query := "INSERT INTO `user` (username, password, email, id) values ($1, $2, $3, $4)"
	var id int
	err := db.QueryRow(query, user.Name, user.Password, user.Email, user.Id).Scan(&id)
	if err != nil {
...
func UpdateUser(db *sql.DB, user *models.User) error {
	query := "Update user SET username=$1, email=$2, password=$3"
